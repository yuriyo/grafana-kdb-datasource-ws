...
export const defaultRowCountLimit = 10000;
export var defaultTimeout = 5000;
export const kdbEpoch = 946684800000;
...
//Graph response type
export const graphFunction: string =
  '{@[x;y;{`payload`error`success!(();"Error! - ",x;0b)}]}{[dict] \n ' +
  ' \n ' +
  ' \n ' +
  ' conc:{ \n ' +
  ...
  ` :{'x}"Time column defined (",` +
  'string[d[`temporal_field]], \n ' +
  ...
  ' if[0=count raw; \n ' +
  ` :{'x}` +
  '"Custom function has generated no data - ", \n ' +
  ...
  ` :{'x}` +
  '"Row limit exceeded, limit set at ",string[control`rowlimit], \n ' +
...
//Table response type
export const tabFunction: string =
  '{@[x;y;{`payload`error`success!(();"Error! - ",x;0b)}]}{[dict] \n ' +
  ' \n ' +
  ' \n ' +
  ' conc:{ \n ' +
...
export const argCounter: string =
  'argcount:{v:value x;$[100h=t:type x;count v 1; \n ' +
