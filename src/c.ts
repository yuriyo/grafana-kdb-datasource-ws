...
export class C {
    ...
    for (var i = 0; i < u8.length; i++) {
      switch ((C = u8[i]) >> 4) {
        ...
        case 14:
          c1 = u8[++i];
          c2 = u8[++i];
          u16 += String.fromCharCode(((C & 0x0f) << 12) | ((c1 & 0x3f) << 6) | ((c2 & 0x3f) << 0));
          break;
      }
    }
    return u16;
  ...
  deserialize(x) {
    ...
    function rSymbol() {
      ...
      return _this.u16u8(s);
    }

    function rTimestamp() {
      return date(rInt64() / 1000000);
    }

    function rMonth() {
      return new Date(Date.UTC(2000, rInt32(), 1));
    }
    ...
    function rSecond() {
      return date(rInt32() / 1000);
    }

    function rMinute() {
      return date(rInt32() * 60000);
    }
  ...
  serialize(x) {
    ...
    function calcN(x, dt) {
      ...
      switch (t) {
        ...
        case 'string':
          return _this.u8u16(x).length + (x[0] === '`' ? 1 : 6);
        case 'date':
          return 9;
        case 'symbol':
          return 2 + _this.u8u16(x).length;
      }
      throw 'bad type ' + t;
    }
    ...
    function w(x, dt) {
      ...
      switch (t) {
        ...
        case 'date':
          {
            wb(-15);
            fb[0] = (x.getTime() - new Date(x).getTimezoneOffset() * 60000) / 86400000 - 10957;
            wn(8);
          }
          break;
        case 'symbol':
