...
export default class ResponseParser {
  ...
  mapTableData(res, req) {
      ...
      for (let col = 0; col < res.payload.columns[0].length; col++) {
        ...
        if (temporalFieldInc) {
          if (req[1].queryParam.temporal_field === req[1].queryParam.column[col][1]) {
            ...
            table.columns[col].text =
              req[1].queryParam.column[col][2] === '::'
                ? req[1].queryParam.column[col][1].replace('`', '')
                : req[1].queryParam.column[col][2];
          }
        } else {
          table.columns[col].text =
    ...
    res.payload.rows[0].forEach(function (rowLoop) {
      ...
      for (let col = 0; col < res.payload.columns[0].length; col++) {
        if (
          req[1].hasOwnProperty('temporal_field') &&
          req[1].queryParam.temporal_field === req[1].queryParam.column[col][1]
        ) {
  ...
  mapGraphData(res, req) {
    ...
    for (let g = 0; g < response.payload[0].length; g++) {
      ...
      for (curCol = 2; curCol <= colKeys.length; curCol++) {
        var fieldName =
          req[1].queryParam.query.type === '`select'
            ? req[1].queryParam.column[curCol - 2][1].replace('`', '')
            : colKeys[curCol - 1];
